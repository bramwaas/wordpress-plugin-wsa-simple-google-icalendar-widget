    ...
    public function simple_ical_info_page_html () {
      ...
      '</p><p><strong>' .
       ...
       echo wp_kses_post('<span id="logging"></span>'.
           ...
           '</strong></p><p>'.
           __('If a request to retrieve calendar data fails, a message is generated.<br>From version 3.0.0 onwards, the Log::log($priority, $message) function is used for logging the messages (formatted as <priority> <category> <message> to error_log() with the category "simple-ical-block" and<br>generally with the priority NOTICE or WARNING, but other priorities may be added later.<br><br>The standard Wordpress constantes (WP_DEBUG=true, WP_DEBUG_LOG=true, WP_DEBUG_DISPLAY=false) and @ini_set( \'display_errors\', 0 ); set in  wp-config.php<br>control the loging of these messages just like other messages.<br>See: https://developer.wordpress.org/advanced-administration/debug/debug-wordpress/', 'simple-google-icalendar-widget').
           '</p><p>'.
           __('Specific for the Log::log() function you can set to constantes in wp-config.php<br>SIB_LOG_MINIMUM_LEVEL to control the minimum priority in the series EMERGENCY,ALERT,CRITICAL,ERROR,WARNING,NOTICE,INFO,DEBUG or ALL to log a message; default WARNING<br>', 'simple-google-icalendar-widget').
           '</p><p>'.
